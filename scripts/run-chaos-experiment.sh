...
kubectl top pods -l app=resilient-app

node_cpu_seconds_total{mode="user"}

process_cpu_seconds_total{job="kubernetes-nodes"}

sum(kube_pod_status_phase{pod=~"resilient-app.*"}) by (phase)

kube_pod_container_status_restarts_total{pod=~"resilient-app.*"}

sum(rate(process_cpu_seconds_total{pod=~"resilient-app.*"}[1m])) by (pod)

sum(rate(container_cpu_usage_seconds_total{pod=~"resilient-app.*"}[1m])) by (pod)

container_memory_working_set_bytes{pod=~"resilient-app.*"} 